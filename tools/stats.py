...
def qdrant_stats(collection: str) -> None:
...
def graph_stats(graph_name: str) -> None:
    import falkordb
    ...
    print(f"\n■ FalkorDB ({graph_name})")
    print(f"  노드                  {one('MATCH (n) RETURN count(n)')}")
    print(f"  REL 엣지              {one('MATCH ()-[r:REL]->() RETURN count(r)')}")
    ...
    for label, q in [
        ...
        ("scope 없음", "MATCH (e:Event) WHERE e.scope IS NULL OR e.scope = '' RETURN count(e)"),
        ("scope_type unknown", "MATCH (e:Event) WHERE e.scope_type = 'unknown' RETURN count(e)"),
        (
            "date_ts 없음/0",
    ...
    print(f"  FOLLOWED_BY           {one('MATCH ()-[r:FOLLOWED_BY]->() RETURN count(r)')}")
    skip = one(
...
def registry_stats(dept: str) -> None:
    if not POSTGRES_URL:
