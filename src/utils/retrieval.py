...
# ── 하이브리드 검색 (벡터 + 어휘) ────────────────────────────────────────────
# **기본 켜짐** (2026-09-14, dev 45문항 A/B 후).
#
# 총점은 근거가 되지 못했습니다 — OFF 0.944 → ON 0.967 (+0.023) 이지만
# 대응표본 t=0.63 으로 노이즈와 구분되지 않습니다. 게다가 **같은 코드로 돌린
# 두 회차의 기준선이 0.956 → 0.944 로 0.012 움직였습니다**(답변 생성
# 비결정성). 이 평가로는 ±0.02 이하를 구분할 수 없습니다.
#
# 켠 근거는 총점이 아니라 **확인된 인과**입니다. Q37 은
#   ① 벡터로 페이지 20위, 서브쿼리 4개 중 3개는 청크 창(80)에도 못 듦
#   ② 원인은 의미 거리 — 재무 문서에 묻힌 운영 세부사항이라, 경쟁하는
#      프로세스 문서들이 문서 수준에서 질문에 더 가까움 (0.58 vs 0.71)
#   ③ BM25 로는 1위 (25.9 vs 22.8) — "워크북" 이 코퍼스에서 희귀어
#   ④ 켜니 0.0 → 1.0 으로 회복
# 예측하고 측정하고 확인한 순서입니다. 함께 오른 Q14·Q48 은 기준선도 흔들린
# 문항이라 노이즈와 구분할 수 없어 근거로 세지 않았습니다.
#
# 비용도 실재합니다 — Q77 을 1.0 → 0.0 으로 잃었습니다. "제시된 테이블에서…"
# 처럼 대상을 특정하지 않는 질문에서, 표가 든 문서가 더 많이 들어오자 모델이
# 고르지 못했습니다. 문항 결함이기도 하지만 **컨텍스트가 늘면 모호한 질문이
# 나빠진다**는 실재하는 부작용입니다.
#
# 끄려면 LEXICAL_ENABLED=0.
#
...
LEXICAL_ENABLED: bool = os.environ.get("LEXICAL_ENABLED", "1").strip().lower() in {
    "1",
    "true",
    "yes",
}
