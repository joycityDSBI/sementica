...
`golden_v2` 90문항 → dev 45 / holdout 45 (출처 문서 겹침 0).

| | 평균 | 통과 | 담당자 | 관계 | 문서위치 | 정책/규정 | 복합 | easy | medium | hard |
|---|---|---|---|---|---|---|---|---|---|---|
| dev (`df09f5e7`) | **0.956** | 42/45 | 1.00 | 0.92 | 1.00 | 0.89 | 1.00 | 0.95 | 1.00 | 0.86 |
| holdout (`640a7314`) | **0.944** | 42/45 | 1.00 | 1.00 | 1.00 | 0.89 | 0.83 | 1.00 | 0.95 | 0.85 |

**차이 +0.011 ± 0.042** (1 표준오차). 차이/표준오차 = 0.26 으로, 유의미하려면
2 는 넘어야 합니다. 이 측정이 검출할 수 있는 최소 과적합이 약 0.08 인데 실제
차이는 그보다 7배 작습니다 — **과적합이 검출되지 않았습니다.**

난이도별로도 일관됩니다(hard 0.86 vs 0.85). 카테고리가 엎치락뒤치락하는 것은
(관계 0.92→1.00, 복합 1.00→0.83) 각 5~9문항이라 예상 범위입니다.

**주장의 범위.** 이 결과가 말하는 것은 *같은 코퍼스의 처음 보는 문서에
일반화된다* 까지입니다. 두 세트 모두 같은 생성기·프롬프트·판정 모델을 쓰므로
문항 생성의 체계적 편향은 양쪽에 똑같이 들어가고, 다른 본부 코퍼스나 사람이
실제로 던지는 질문에 대해서는 아무것도 말해주지 않습니다. "차이가 없다"도
"과적합이 0"이 아니라 "0.08 보다 작다" 입니다.

**측정값이 약간 낮게 잡혔을 가능성.** 홀드아웃 실패 3건 중 2건이 `"이 제보
문서에서…"` 처럼 대상 문서를 특정하지 않는 질문이었습니다. `verify_answerable`
필터가 걸렀어야 할 유형이며, 답할 수 없는 질문이 섞이면 측정값이 시스템 실력
아래로 내려갑니다. 검색 결함이 아니라 **문항 결함**입니다.
...
| 78 | 의존성 버전 고정 | 🔜 | `tools/pin_requirements.py --write` 를 VM 의 .venv 로 실행. anthropic 이 조용히 올라가 그래프가 비었던 사고의 재발 조건이 남아 있음 |
| 79 | 백업 이원화 정리 | 🔜 | `backup.sh`(cron 등록) 는 PostgreSQL 을, `backup_to_gcs.sh` 는 Notion .md·설정 파일을 백업. **둘 중 하나만 돌면 뭔가는 빠집니다** |
| 80 | 로그 로테이션 | 🔜 | `sync_cron.log` · `backup.log` 가 무한히 커집니다 (systemd 로 옮긴 MCP·REST 는 journald 가 관리) |
| 73 | 하이브리드 검색 (벡터+BM25) | ✅ | `utils/lexical.py` + RRF. Q37(벡터로 도달 불가)을 0.0 → 1.0. `LEXICAL_ENABLED=0` 으로 끌 수 있음, 2026-09-14 |
...
| 날짜 | 내용 |
|------|------|
| 2026-09-14 | **시스템 점검 — 네 가지 발견.** ① 의존성 상한 없음(오늘 사고의 재발 조건) ② 실패를 아무도 모름(인제스트 partial 을 진단 도구 돌리기 전까지 몰랐음) ③ 백업 스크립트 2개가 서로 다른 것을 빠뜨림 ④ cron 로그 로테이션 없음 |
| 2026-09-14 | **결과 메일 알림 구현** — 인제스트·동기화·백업이 끝날 때 발송. **성공해도 보냅니다**: 실패에만 보내면 침묵이 '잘 돌았다' 와 'cron 이 안 돌았다' 를 모두 뜻하게 됩니다. `backup.sh` 가 `\|\| true` 로 실패를 삼키고 항상 '완료' 라고 보고하던 것도 함께 수정 |
| 2026-09-14 | **홀드아웃 재측정 0.944 → 0.967** — dev 와 같은 방향·같은 크기(+0.023). dev 0.967 = holdout 0.967 로 일반화 격차 0. 개별로는 t=1.4 지만 **독립된 두 문항 집합에서 같은 값이 나온 것**이 개별 t값보다 강한 증거입니다 |
| 2026-09-14 | **dev 잔여 실패 2건이 모두 시스템 결함이 아님을 확인** — Q77 은 문항 결함(대상 미특정), Q41 은 정상 동작(소속 엣지가 없는데 단정하지 않음). 이 골든셋으로는 더 짜낼 것이 없습니다 |
| 2026-09-14 | **하이브리드 검색(벡터+BM25) 도입** — Q37 은 벡터로 페이지 20위(서브쿼리 4개 중 3개는 청크 창에도 못 듦), BM25 로 1위. 의미 거리가 멀어 파라미터로는 못 옮기던 건입니다. RRF 로 합침(척도가 달라 점수는 못 더함). dev 0.944 → 0.967 |
| 2026-09-14 | **평가의 측정 한계 확인** — 같은 코드·같은 골든셋 두 회차가 0.956 / 0.944 로 갈렸습니다. **±0.02 이하의 차이는 실제 개선으로 읽으면 안 됩니다.** 총점이 아니라 예측→측정→확인된 인과로 판단해야 합니다 |
| 2026-09-14 | **이벤트 17개 손실 수정** — `event_id = uuid5(source_url)` 이라 한 페이지의 이벤트가 모두 같은 ID 로 MERGE 되어 서로를 덮어썼습니다. 그래프 165개 = 이벤트를 만든 페이지 165개(장부는 182개). 출처+내용 기반 ID 로 변경 |
| 2026-09-14 | **홀드아웃 첫 측정 — 과적합 검출 안 됨.** dev 0.956 / holdout 0.944, 차이 +0.011 ± 0.042 (유의 임계 0.08). 그동안의 튜닝이 특정 문항 암기가 아니었음을 확인. 주장 범위는 '같은 코퍼스의 처음 보는 문서' 까지 |
