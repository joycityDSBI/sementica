...
| 설정 | 근거 포함률 | 판단 |
|------|-----------|------|
| oversample 1 | 57.5% | ❌ −20%p |
| oversample 2 | 70.0% | ❌ −7.5%p |
| **oversample 4** | **77.5%** | ✅ **현행 유지** |
| oversample 8 | 77.5% | 이득 없음 |
| coverage boost 0.0 / 0.10 / 0.20 | 모두 77.5% | recall 무영향 |
| dedupe 0.85 | 70.0% | ❌ **적용 안 함** |

**바로잡은 판단 두 가지**

- **청크 오버샘플은 효과가 있습니다.** 도입 당시 "효과 없음"으로 기록했으나,
  그때는 페이지 절단 문제가 효과를 가리고 있었습니다. 지금 끄면 20%p 손해입니다.
...
| 45 | 청크 오버샘플 검증 | ✅ | 4가 적정 (1배 대비 +20%p). 이전 "효과 없음" 판단 정정, 2026-09-10 |
| 46 | near-duplicate 제거 | ⛔ | A/B 결과 −7.5%p — **적용하지 않음**. 함수는 남기되 기본 비활성 |
| 47 | 벡터 검색 실패 문항 개선 | 🔜 | 벡터 의존 카테고리 recall 77.5% — 임베딩·청킹 개선 여지 |
...
| 날짜 | 내용 |
|------|------|
| 2026-09-10 | **검색 파라미터 A/B** (`tools/ab_retrieval.py`) — 오버샘플 4 적정 확인(이전 "효과 없음" 판단 정정), near-duplicate 제거는 −7.5%p 로 **미적용** 결정 |
