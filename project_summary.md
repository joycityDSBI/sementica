...
| 설정 | 근거 포함률 | 판단 |
|------|-----------|------|
| ~~oversample 1~~ | ~~57.5%~~ | ⚠️ **무효 — 재측정 필요** |
| ~~oversample 2~~ | ~~70.0%~~ | ⚠️ **무효 — 재측정 필요** |
| ~~oversample 4 (기준)~~ | ~~77.5%~~ | ⚠️ **무효 — 재측정 필요** |
| ~~oversample 8~~ | ~~77.5%~~ | ⚠️ **무효 — 재측정 필요** |
| coverage boost 0.0 / 0.10 / 0.20 | 모두 동일 | recall 무영향 (유효) |
| dedupe 0.85 | −7.5%p | ❌ **적용 안 함** (유효) |

> ⚠️ **oversample 행은 잘못 측정된 값입니다 (2026-09-10 정정).**
> 측정 도구가 낮은 oversample 을 "캐시된 페이지 목록을 앞에서 N개만 남기는"
> 방식으로 흉내냈는데, `vector_search_pages` 에서 `limit` 은 **페이지** 수이고
> `oversample` 은 후보 **청크** 풀만 넓힙니다. 그래서 저 숫자들은
> "페이지를 1/2/5/10개 넘겼을 때의 recall" 이지 oversample 의 효과가 아닙니다.
> 기준 행조차 5페이지로 돌아, 운영(10페이지)과 다른 조건이었습니다.
> 도구는 oversample 값마다 실제 검색을 다시 하도록 고쳤고(임베딩은 재사용),
> **`CHUNK_OVERSAMPLE=4` 는 현재 근거 없이 유지 중인 값입니다.** 재측정 필요.
>
...
- **near-duplicate 제거는 해롭습니다.** 정작 고치려던 Q36 을 포함해 3문항을
...
- **청크 오버샘플에 대한 "효과 있음" 정정은 철회합니다.** 도입 시 "효과 없음",
  A/B 후 "효과 있음"으로 두 번 판단을 뒤집었는데, 두 번째 근거가 위와 같이
  무효였습니다. 현재로선 **모릅니다.**
...
| 45 | 청크 오버샘플 검증 | ⚠️ | 측정 오류로 **무효** — 도구 수정 후 재측정 필요, 2026-09-10 |
| 46 | near-duplicate 제거 | ⛔ | A/B 결과 −7.5%p — **적용하지 않음**. 함수는 남기되 기본 비활성 |
| 47 | 벡터 검색 실패 문항 개선 | 🔜 | 벡터 의존 카테고리 recall 개선 — 임베딩·청킹 여지 |
...
| 50 | 오버샘플 재측정 | 🔜 | 수정된 `ab_retrieval.py` 로 1/2/4/8 재측정 후 `CHUNK_OVERSAMPLE` 확정 |
| 51 | 유실 페이지 점검 | 🔜 | `notion_pages` 에서 `chunk_count=0 AND status='ok'` 조회 → 재처리 대상 확인 |
...
| 2026-09-10 | **A/B 측정 오류 정정** — oversample 을 결과 슬라이싱으로 흉내내 실제로는 "페이지 개수"를 재고 있었음. 도구 수정, `CHUNK_OVERSAMPLE=4` 근거는 **무효** 처리. boost·dedupe 결론은 유효 |
