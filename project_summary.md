...
| 81 | 주기 실행 Airflow 전환 | ✅ | `dags/` — 일일(sync→backup) · 주간(용어집→dev 평가). cron 의 시계 기반 순서 가정을 실제 의존성으로 교체. local/ssh 실행 모드 모두 지원, 2026-09-15 |
| 80 | 로그 로테이션 | ✅ | `deploy/logrotate-sementica` — cron 로그만 대상(주간 + 50M 초과 시, 8회 보존). systemd 서비스는 journald 소관이라 제외, 2026-09-15 |
