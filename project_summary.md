...
> 최종 업데이트: 2026-09-10 (40문항 골든셋 **0.950** — 예산 기반 컨텍스트, 검색 경로 통합)
...
easy 1.00 (14) / medium 0.97 (18) / **hard 0.81** (8) 입니다.
...
| 44 | 청크 오버샘플 재검토 | 🔜 | 순효과 미확인. `CHUNK_OVERSAMPLE` A/B 필요 |
| 45 | VM 용어집 네트워크 복구 | 🔜 | `catalog.joycityplay.com` 접근 차단 (curl → 000). 스냅샷으로 우회 중 |
| 46 | near-duplicate 문서 제거 | 🔜 | 같은 제목의 유사 문서가 컨텍스트를 중복 점유 (Q36 사례) |
...
| 날짜 | 내용 |
|------|------|
| 2026-09-10 | 분해 시 **원본 질문도 검색 대상에 포함** — 긴 엔티티 이름이 축약되어 그래프 노드 매칭이 깨지던 문제 |
